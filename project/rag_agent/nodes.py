...
# --- ADDED: Imports for the Integrity Module ---
from sentence_transformers import CrossEncoder
import numpy as np

# Load the lightweight NLI transformer (runs on CPU or minimal GPU memory)
# This acts as your patented secondary transformer for the obligation delta
delta_model = CrossEncoder('cross-encoder/nli-deberta-v3-small') 
# -----------------------------------------------
...
# --- ADDED: The Legal Weight Integrity Module Node ---
def integrity_check(state: AgentState):
    """
    Evaluates the obligation delta between the retrieved context and the generated summary.
    """
    original_text = state.get("context_summary", "").strip()
    simplified_text = state["messages"][-1].content
    
    # If there is no context yet, skip the guardrail
    if not original_text or not simplified_text:
        return {}

    # Calculate Obligation Delta using the secondary transformer
    raw_scores = delta_model.predict([(original_text, simplified_text)])
    
    # Convert logits to probabilities to get a clean 0.0 to 1.0 delta score
    probabilities = np.exp(raw_scores[0]) / np.sum(np.exp(raw_scores[0]))
    contradiction_prob = probabilities[0] 
    
    # --- ADDED: Print the score to the terminal every time so you can monitor it ---
    if contradiction_prob > 0.5:
        print(f"\n❌ INTEGRITY FAIL! Delta Score: {contradiction_prob:.2f} (Routing back to Llama...)")
        feedback = (
            "SYSTEM GUARDRAIL TRIGGERED: Your previous summary stripped away critical "
            "legal obligations, rights, or penalties found in the original text. "
            "Rewrite the summary, ensuring NO legal duties or liabilities are lost or hallucinated."
        )
        return {"messages": [HumanMessage(content=feedback)]}
    else:
        print(f"\n✅ INTEGRITY PASS. Delta Score: {contradiction_prob:.2f} (Sending to user)")
        return {}
# -----------------------------------------------------
